...
def test_process_image_no_blur_generated(tmp_dirs, sample_jpeg):
    """process_image should not return blur_filename (blur is handled by CSS)."""
    result = process_image(
        sample_jpeg, "photo.jpg",
        originals_dir=tmp_dirs["originals"],
        thumbnails_dir=tmp_dirs["thumbnails"],
    ...
    assert "blur_filename" not in result
