...
- **Deploy is clean-slate**: `deploy.sh` uses `docker compose down -v`, backs up originals, rebuilds from scratch, then re-uploads originals through the API (so they get reprocessed with new code). The backup is verified and removed on success. `--seed-from <local-dir>` uploads originals from the dev machine instead, for a host with no data to back up.
- **Prod host is Ubuntu**: home-pc runs Ubuntu 26.04 with Docker Engine from apt (`docker.io` + `docker-compose-v2`). `deploy.sh` assumes a POSIX shell over SSH (`/home/home/photo-frame`, `find`, `rm -rf`). `docker` works without sudo via the `docker` group; a fresh SSH session is needed for group membership to apply.
